...
```{r}
# Ensure both results data frames have the same k values for merging
combined_results <- merge(cv_results, test_results, by = "k", suffixes = c("_cv", "_test"))

# You may want to plot EDF vs. MSE, so let's calculate EDF for cv_results as well
combined_results$edf_cv <- nrow(data) / combined_results$k
combined_results$edf_test <- nrow(data) / combined_results$k

```


```{r}
# Plot Cross-Validation and Test MSE vs. k
ggplot(combined_results, aes(x = k)) +
  geom_line(aes(y = MSE_cv, color = "Cross-Validation MSE"), size = 1) +
  geom_line(aes(y = Test_MSE, color = "Test MSE"), size = 1) +
  labs(title = "Cross-Validation and Test MSE vs. k",
       x = "Number of Neighbors (k)",
       y = "Mean Squared Error (MSE)") +
  scale_color_manual(name = "Legend", values = c("Cross-Validation MSE" = "blue", "Test MSE" = "red"))
```
